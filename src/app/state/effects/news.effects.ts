...
export class NewsEffects {
  ...
  createNews$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap((news) =>this.newsService.createNew(news)
      .pipe(
        map(news => actions.createdNews(news),
        catchError(() => of({ type: '[News API] News Loaded Error' }))
      ))
    )
  )
  ...
  deleteNews$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap(({id}) =>this.newsService.deleteNew(id)
      .pipe(
        map(news => actions.deletedNews(),
        catchError(() => EMPTY)
      ))
    )
  )
